    ...
    public class WaveFunctionCollapseController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (m_FrameInput.Restart)
            {
                ...
                {
                    m_CTS = new CancellationTokenSource();
                    StartWaveAsync(m_CTS).Forget();
                }
            }
        }

        private async UniTask StartWaveAsync(CancellationTokenSource cts)
        {
            await m_WaveFuncionCollapse.GenerateAuto(m_AutoGenerateDelayMS, cts, () => Debug.Log("callback working"));
        }
